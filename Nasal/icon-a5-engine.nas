...
#
#   Systems/icon-a5-XL-RG-engine.xml
#
...
var ENGINE_DATA		= "icon-a5/engine/";		# Base location for engine data
...
var engine_update = func (i) {						# i = engine number
  ...
  if (getprop(eprop~"running") == 1) {					# Is engine running?
    ...
    if (use_boostpump) {						# Many boost pump scenarios will richen AFR a bit. This is one reason why you should
      ...
      if (getprop("/systems/electrical/outputs/fuel-pump")) { 
...
var engine_setup = func {
  ...
  var table = {};
  table = props.globals.getNode("/icon-a5/engine/table-mixture-correction").getChildren("mix");
